...
class F1SeasonApp:
    VALID_F1_NAMES = ["Red Bull", "Mercedes"]
    ...
    def register_team_for_season(self, team_name: str, budget: int):
        if team_name in F1SeasonApp.VALID_F1_NAMES:
    ...
    def new_race_results(self, race_name: str, red_bull_pos: int, mercedes_pos: int):
        if not all([self.mercedes_team, self.red_bull_team]):
            raise Exception("Not all teams have registered for the season.")
        better_position_team = "Red Bull" if red_bull_pos < mercedes_pos else "Mercedes"
